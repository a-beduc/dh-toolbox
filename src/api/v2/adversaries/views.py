...
class AdversaryDetailApi(APIView):
    ...
    class BasicAttackOut(serializers.Serializer):
        ...
        def get_damage(self, obj):
            ...
            if dmg is None:
                return None
            return AdversaryDetailApi.DamageOut(dmg).data
    ...
    class OutputSerializer(serializers.Serializer):
        ...
        def get_author(self, obj):
            return self._serialize(AdversaryDetailApi.AuthorOut, obj.author)

        def get_basic_attack(self, obj):
            ba = getattr(obj, "basic_attack", None)
            return self._serialize(AdversaryDetailApi.BasicAttackOut, ba)

        def get_features(self, obj):
            return self._serialize_many(
                AdversaryDetailApi.FeatureOut,
                obj.features.all()
            )

        def get_experiences(self, obj):
            return self._serialize_many(
                AdversaryDetailApi.ExperienceOut,
    ...
    def get(self, request, adversary_id):
        ...
        if adv is None:
            raise Http404
        data = self.OutputSerializer(adv, context={"request": request}).data
...
class AdversaryListApi(APIView):
    ...
    def get(self, request):
        ...
        data = self.OutputSerializer(adversaries,
                                     many=True,
